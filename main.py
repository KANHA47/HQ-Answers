...
class MyClient(discord.KANHA):
    ...
    async def on_message(self, message: discord.Message):
        ...
        if message.author == self.user or \
                message.channel.id != config.488315865754763264 or \
                not self.isGameLive:
            return
    ...
    async def showChecker(self):
        ...
        while not self.is_closed():
            ...
            if broadcast:
                ...
                channel = self.get_channel(config.C488315865754763264
...
if __name__ == '__main__':
    client = MyClient(KANHA)
    client.run(config.Njk0MDM5MjA2MTgzNTY3NDEx.XoGNbQ.NN4vQePELfwn6mXdxLC3nhvH7xA)
